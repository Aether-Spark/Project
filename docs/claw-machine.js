...
class Toy extends WorldObject {
  ...
  collectToy(toy) {
    ...
    const collectedToy = Object.assign(document.createElement('div'), {
      className: `toy-wrapper ${settings.collectedNumber > 6 ? 'squeeze-in' : ''}`,
      innerHTML: `<div class="toy pix ${toy.toyType}"></div>`
    });
    
    collectedToy.addEventListener('click', () => {
      showCongratsMessage(`🎉 Kamu mendapatkan ${toy.toyType.toUpperCase()}!`);
    });
    
    elements.collectionBox.appendChild(collectedToy);
...
function showCongratsMessage(text) {
  ...
  message.className = "congrats-message show";
  message.textContent = text || "🎉 Selamat! Kamu dapat boneka!";
